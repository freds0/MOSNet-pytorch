...
class getdataset(torch.utils.data.Dataset):
    def __init__(self, config, seed, mode):
        ...
        if mode == "train":
            self.filelist = read_list(os.path.join(config["data_dir"], 'train.csv'))
        elif mode == "valid":
    ...
    def getmax_timestep(self, config, seed):
        file_list_train = read_list(os.path.join(config["data_dir"], 'train.csv'))
        file_list_val = read_list(os.path.join(config["data_dir"], 'val.csv'))
        file_list_test = read_list(os.path.join(config["data_dir"], 'test.csv'))
        file_list = file_list_train + file_list_val + file_list_test
        random.seed(seed)
    ...
    def __getitem__(self, index):
        # Read audio
        filename, mos, _, _ = self.filelist[index].split(',')
