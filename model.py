...
class BLSTM(nn.Module):
    '''
    QualityNet Architecture
    '''
    def __init__(self):
        super(BLSTM, self).__init__()
        self.blstm1 = nn.LSTM(257, 128, bidirectional=True, batch_first=True)
        self.droupout = nn.Dropout(0.3)
        # FC
        self.flatten = TimeDistributed(nn.Flatten(), batch_first=True)
        self.dense1 = nn.Sequential(
            TimeDistributed(nn.Sequential(nn.Linear(in_features=257, out_features=128), nn.ReLU()), batch_first=True),
            nn.Dropout(0.3))
    ...
    def forward(self, forward_input):
        ...
        # blstm
        blstm_output, (h_n, c_n) = self.blstm1(forward_input)
        blstm_output = self.droupout(forward_input)
